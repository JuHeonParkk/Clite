...
public class Parser {
    ...
    public Program program() {
        ...
        Block b = progstatements(); // student exercise
    ...
    private Value literal( ) {
    	Value value = null;
    	String stv = token.value();
    	if (token.type().equals(TokenType.IntLiteral)) {
    		value = new IntValue(Integer.parseInt(stv));
    		token = lexer.next();
    	} else if (token.type().equals(TokenType.FloatLiteral)) {
    		value = new FloatValue(Float.parseFloat(stv));
    		token = lexer.next();
    	} else if (token.type().equals(TokenType.CharLiteral)) {
    		value = new CharValue(stv.charAt(0));
    		token = lexer.next();
    		...
    		error("Erro in literal value contruction");
    	}
